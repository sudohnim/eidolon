...
class Maigret(Tool[MaigretInput, MaigretOutput]):
    ...
    #: checks 3155 sites for a username — no single vendor host
    vendor = ""

    def _input_value(self, inp: MaigretInput) -> str:
        return inp.username

    def to_findings(self, out: MaigretOutput) -> list[Finding]:
        """One Account per claimed profile. A username claim is NOT a
        confirmed registration (anyone could hold the name), so active=False —
        merge precedence keeps a holehe/blackbird confirmation when both exist."""
        return [
            Account(
            ...
            for p in out.profiles_found
            if p.platform
        ]

    def run_summary(self, out: MaigretOutput) -> str:
        return (
            f"{out.found_count} profiles found across "
            f"{out.platforms_checked} platforms"
        )
