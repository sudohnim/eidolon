...
class Blackbird(Tool[BlackbirdInput, BlackbirdOutput]):
    ...
    #: subprocess tool probing 600+ platforms — no single vendor host
    vendor = ""
    ...
    def to_findings(self, out: BlackbirdOutput) -> list[Finding]:
        """One Account per found registration (probed endpoints confirm the
        address is registered, so active=True)."""
        return [
            Account(
            ...
            for a in out.accounts_found
            if a.platform
        ]

    def run_summary(self, out: BlackbirdOutput) -> str:
        return f"{out.found_count} accounts found"
